...
import (
	...
	staticpages "github.com/lmnzx/staticpages/api/v1"
...
func init() {
	utilruntime.Must(staticpages.AddToScheme(scheme))
}
...
func (r *reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	var staticPage staticpages.StaticPage
	...
	if err != nil {
		if k8serrors.IsNotFound(err) {
			...
			if err != nil {
				return ctrl.Result{}, fmt.Errorf("couldn't deelte deployment: %s", err)
			}
	...
	if err != nil {
		if k8serrors.IsNotFound(err) {
			...
			deploymentsObj := getDeploymentObject(staticPageName, staticPage.Spec.Image, staticPage.Spec.Replicas)
			_, err := deploymentsClient.Create(ctx, deploymentsObj, metav1.CreateOptions{})
			if err != nil {
				return ctrl.Result{}, fmt.Errorf("couldn't create deployment: %s", err)
			}
			log.Info("new staticpage wite name " + staticPageName + "created")
			return ctrl.Result{}, nil
		} else {
